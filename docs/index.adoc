= Introduction to Headless CMS for Enonic developers
:toc: right
:experimental:

This guide provides a hands on introduction to using XP in Headless CMS mode.
...
Throughout this guide, we will be using the headless CMS demo application https://market.enonic.com/vendors/enonic/headless-movie-db[Headless Movie Data Base]" aka HMDB as our foundation. The app may also be installed to your XP installation directly from Enonic Market.
...
Your newly created app includes a sample dataset. When the application started, it automatically created a project called "Headless Demo" and imported the `HMDB` dataset.

Simply select this project, and you will see the content as seen in the screenshot below:
...
To access and browse the API, simply point your browser to: http://localhost:8080/site/default/draft/hmdb/api

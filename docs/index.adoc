...
This is a basic introduction to using Enonic XP as a Headless CMS.
...
In this guide, we are using the demo application https://market.enonic.com/vendors/enonic/headless-movie-db[Headless Movie Data Base] aka HMDB as a template for our project.
...
TIP: Leave out the `-r app-hmdb` option from the command to get the list of available starters.
